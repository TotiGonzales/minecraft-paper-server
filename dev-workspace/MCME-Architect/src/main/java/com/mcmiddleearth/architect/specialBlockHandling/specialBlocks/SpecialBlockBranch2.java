...
public class SpecialBlockBranch2 extends SpecialBlock {
    ...
    public static SpecialBlockBranch2 loadFromConfig(ConfigurationSection config, String id) {
        ...
        diagonalData = Bukkit.createBlockData("minecraft:prismarine_wall[east=low,north=none,south=none,up=false,waterlogged=false,west=none]");
        for(int k = 0; k < eightFaces.length; k+=2) {
        ...
        return new SpecialBlockBranch2(id, horizontal, sloped, wall, verticalThin, width, slope, SpecialBlockType.BRANCH);
    }

    protected SpecialBlockBranch2(String id, BlockData[][] horizontal, BlockData[][][] sloped, BlockData wall,
    ...
    private BlockState getBlockState(Block blockPlace, BlockFace playerFace, int width, int slope, boolean negativeSlope) {
        BlockData blockData;
        if(slope == horizontal) {
            ...
            if(negativeSlope) {
                blockData = getBlockData(eightFaces, blockDataSloped[width][slope], playerFace.getOppositeFace());
            } else {
    ...
    public Block getBlock(Block clicked, BlockFace blockFace,
        ...
        boolean negativeSlope = player.getLocation().getPitch()<-22.5;
...
//Logger.getGlobal().info("Interaction Point: "+interactionPoint);
        if(negativeSlope) {
            ...
            //lower part clicked
                ...
                if(isSteep(clicked.getBlockData())) block = block.getRelative(BlockFace.DOWN);
                BlockFace steepClicked = getSteepSlopedMainOrientation(clicked.getBlockData());
        ...
        //positive slope
            if (isUpperPlace(interactionPoint, blockFace)) {
                ...
                if (slope == steep) {
                    ...
                    if (steep != null) {
                        ...
                        if (diagonal != null) {
                            block = block.getRelative(diagonal);
                        } else if(isHorizontal(clicked.getBlockData())){
                            block = block.getRelative(BlockFace.DOWN);
                            block = block.getRelative(playerFace);
                        }
                    }
            ...
            //lower part clicked
                ...
                if (slope == steep) {
                    ...
                    BlockFace steep = getSteepSlopedMainOrientation(clicked.getBlockData());
                    if (steep != null) {
                        block = block.getRelative(steep.getOppositeFace());
                        block = block.getRelative(BlockFace.DOWN);
                    }
    ...
    private boolean isDiagonalFace(BlockFace face) {
        switch(face) {
            case NORTH_EAST:
            case NORTH_WEST:
            case SOUTH_EAST:
...
//Logger.getGlobal().info("Found connection: " + connection.getLocation());
    ...
    private boolean isUpperPlace(Location interactionPoint, BlockFace face) {
        return interactionPoint==null
                || face.equals(BlockFace.UP)
                || interactionPoint.getY()-interactionPoint.getBlockY()>0.5;
    }
    ...
    private boolean isHorizontal(BlockData data) {
        for(int j = thin; j<= thick; j++) {
            for (int i = 0; i < fourFaces.length; i++) {
                if (data.matches(blockDataHorizontal[j][i])) {
                    return true;
                }
            }
        }
        ...
        if(width >= 0 && player.isSneaking()) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    final BlockState tempState = getBlockState(blockPlace, playerFace, finalWidth, finalSlope, negativeSlope);
                    new BukkitRunnable() {
                        @Override
                        public void run() {
    ...
    private boolean isSteep(BlockData search) {
        for(int i = thin; i <= thick; i++) {
            for(int j = 0; j<eightFaces.length; j++) {
                if(search.matches(blockDataSloped[i][steep][j])) {
                    return true;
                }
            }
